<template>
  <div>
    <a-modal
      ...
      class="gvb_chat_modal"
      width="35%"
    >
      <div class="gvb_chat_body">
        <div
          class="message"
          v-for="(item, index) in data.record_list"
          :key="index"
        >
          <div :class="{ me: item.is_me }">
            <div class="avatar"><img :src="item.send_user_avatar" /></div>
    ...
    >
      <template #cell="{ column, record }">
        <template v-if="column.key === 'send_user'">
          <img
            :src="record.send_user_avatar"
            height="40"
            style="border-radius: 50%"
          />
          <span style="margin-left: 10px">{{
            record.send_user_nick_name
          }}</span>
        </template>
        <template v-if="column.key === 'rev_user'">
          <img
            :src="record.rev_user_avatar"
            height="40"
            style="border-radius: 50%"
          />
      ...
      .message-main {
        .message-content {
          .txt-message::before {
            ...
            border-width: 10px 20px;
            border-style: solid;
            background-color: transparent transparent transparent #d2d2d2;
          }
        }
      }
    }
