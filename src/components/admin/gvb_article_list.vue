<template>
    <div>
      ...
      >
        ...
        <template #edit="record">
          <a-button type="primary" @click="showEditArticleModal(record)"
            >编辑</a-button>
          <a-button type="primary" @click="showEditArticleContentModal(record)"
            >正文编辑</a-button>  
        </template>
        <template #cell="{ column, record, index }">
          ...
          <template v-if="column.key === 'data'">
            <div class="gvb_article_data">
              ...
              <span
                ><i class="fa fa-thumbs-o-up"></i>{{ record.digg_count }}</span
              >
              <span
                ><i class="fa fa-comment-o"></i>{{ record.comment_count }}</span
              >
  ...
  function showEditArticleModal(record) {
    data.state.title = record.record.title;
    data.state.abstract = record.record.abstract;
    data.state.banner_id = record.record.banner_id;
    data.state.category = record.record.category;
    data.state.link = record.record.link;
    data.state.source = record.record.source;
    data.state.tags = record.record.tags;
    data.editID = record.record.id;
